...
int main(int, char *[])
{
  ...
  for (int step = 0; step < steps; ++step)
  {
    ...
    print(input);
    std::cout << "---------------------" << std::endl;
    auto SumOfElements = 0;
    std::for_each(input.begin(), input.end(), [&SumOfElements](auto &line)
                  {
                    std::for_each(line.begin(), line.end(), [&SumOfElements](int &val)
                                  {
                                    if (val > 9)
                                    {
                                      val = 0;
                  ...
                  if(SumOfElements == 0) {
                    std::cout << "Syncronized after: "<< step+1 << std::endl;
                    break;
                  }
  }
